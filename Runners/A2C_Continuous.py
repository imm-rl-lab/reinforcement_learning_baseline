...
def run(attempt, directory, env_name, dt, en, sn, gamma, lrpi, lrv, ent):
    ...
    #Agent
    noise = UniformNoise(action_dim=env.action_dim, threshold_decrease=1/(en * sn))
    pi_model = SequentialNetwork([env.state_dim, 128, 1], nn.ReLU())
    v_model = SequentialNetwork([env.state_dim, 128, 1], nn.ReLU())
    agent = A2C_Continuous(env.action_min, env.action_max, pi_model, v_model, noise, gamma=gamma,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--directory', type=str, default='../Data/A2C_Continuous')
