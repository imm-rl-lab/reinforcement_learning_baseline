...
from Agents.DRQN import DRQN_PartialHistory
...
def run(attempt, directory, env_name, dt, an, lr, en, gamma, bs, hl, ml, tau):
    # set seed
    seed(attempt)
    ...
    DRQN = ContinuousAgentMaker(DRQN_PartialHistory)
    ...
    agent = DRQN(q_model, noise, action_values=action_values, history_len=hl, q_model_lr=lr, gamma=gamma, batch_size=bs,
                 session_memory_len=ml, tau=tau)
...
if __name__ == "__main__":
    ...
    run(args.attempt, args.directory, args.env_name, args.dt, args.an, args.lr, args.en, args.gamma, args.bs, args.hl,
        args.ml, args.tau)
