...
class ContinuousAgentMaker():
    '''The class convert an agent for a discrete action space 
    to the agent for the continuous action space'''
    def __init__(self, DiscreteAgent):
        self.DiscreteAgent = DiscreteAgent
        return self.get_continuous_agent()
        
    def get_continuous_agent(self):

        class ContinuousAgent(self.DiscreteAgent):
