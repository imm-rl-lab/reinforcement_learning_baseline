...
class CEM():
    def __init__(self, pi_model, pi_model_lr, tau, percentile_param, 
    ...
    def fit(self, sessions):
        ...
        elite_states, elite_actions, elite_session_n = self.get_elite_sessions(sessions)
    ...
    def get_elite_states_and_actions(self, sessions):
        ...
        for session in sessions:
            if sum(session['rewards']) >= total_reward_threshold:
                elite_states.extend(session['states'][:-1])
...
class CEM_Discrete(CEM):
    def __init__(self, pi_model, noise, tau=1e-2, percentile_param=70, 
                 learning_iter_per_fit=16, greedy_policies=False):

        super().__init__(pi_model, pi_model_lr, tau, 
    ...
    def get_action(self, state):
        if np.random.uniform() < self.noise.threshold:
            ...
            probs = self.get_probs(model, state)
